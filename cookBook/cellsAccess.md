...
В результате работы скрипта в консоль выводится склеенные в одну строку наименования заголовков строк, так как движок скриптов 1.0 [не переносит курсор](../appendix/constraints.md#no-line-break) на следующую строку после работы функции `console.log()`:
