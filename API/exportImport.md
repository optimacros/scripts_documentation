...
setEnclosure(enclosure: string): Exporter
```
Устанавливает обрамляющий символ, которым будет обрамляться текстовое поле. Обрамление работает всегда, кроме использования `StorageExporter`: для этого интерфейса обрамляются только те поля, в которых содержится разделитель полей или перенос строки. Допустимые значения: `'` и `"`. По умолчанию: `"`.
...
setEscape(escape: string): Exporter
```
~Устанавливает~ символ для экранирования обрамляющего символа, если встретится в строке ещё и он, ~и символа переноса строки~. Допустимые значения: `\` и `"`. ~По умолчанию~ На данный момент метод ничего не устанавливает и экранирующий символ **всегда** равен обрамляющему символу, перенос строки не экранируется.
