...
```
Интерфейс для работы с лицензиями воркспейса: получения информации об установленной на воркспейсе лицензии, создании шифрованной лицензионной строки и её валидации. 

Для создания лицензии требуется пароль (произвольная строка) и ключ, сгенерированный на основе этого пароля. Далее текст лицензии, который должен быть валидным `JSON` произвольного содержания, шифруется с помощью пароля и ключа. После этого пароль и ключ зашиваются в дистриутив, который будет установлен на сервере клиента, а зашифрованная лицензия передаётся клиенту и должна быть введена в панели администратора воркспейса в специальное поле в настроках воркспейса.
...
getWorkspaceLicenseStatus(): boolean
```
Если на воркспейсе в панели администратора установлена лицензия, соответствующая зашитим в дистрибутив паролю и ключу, возвращает `true`. В противном случае возвращает `false`.
...
getWorkspaceLicenseInfo(): object
```
Если на воркспейсе установлена валидная лицензия, возвращает стандартный JS-объект с полями, указанными в исходной структуре лицензии. Если лицензия не установлена, возникает ошибка интепретатора, которая не отлавливается в скрипте.
...
createKey(password: string): string
```
Создаёт ключ на основе пароля, которым можно в дальнейшем подписать лицензию. Повторный вызов приводит к разным результатам.
...
validateKey(password: string, key: string): boolean
```
Проверяет ранее созданный ключ на соответствие паролю и стандарту шифрования. Возвращает `true`, если проверка пройдена, или выбрасывает исключение.
...
validateLicenseJson(jsonStr: string): boolean
```
Проверяет текстовое представление структуры лицензии на соответствие формату `JSON`, никак не проверяет содержимое лицензии. Возвращает `true` или выкидывает исключение об ошибке синтаксиса. 
...
createLicense(password: string, key: string, jsonStr: string): string
```
Создаёт лицензию — зашифрованные данные на основе пароля, ключа и JSON-данных лицензии. Возвращает строку с лицензией. В случае несоответствия ключа паролю или при передаче не `JSON`-строки возникает ошибка интепретатора, которую нельзя отловить внутри скрипта.
...
validateLicense(password: string, key: string, licenseData: string, id?: string | null): object
```
Проверяет ранее созданную лицензию на соответствие паролю и ключу и на соответсвие стандарту шифрования. Возвращает стандартный JS-объект с полями, указанными в исходной структуре лицензии, но у каждого названия свойства объекта появляется префикс `$`. В случае несоответствия пароля ключу возникает ошибка интепретатора, которую нельзя отловить внутри скрипта. В случае нессответствия лицензии ключу и паролю возвращается объект с текстом ошибки: `{"$errors":"Содержание лицензии не распознается"}`.
