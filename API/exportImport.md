...
setDelimiter(delimiter: string): CSVParams
```
Устанавливает разделитель полей. При использовании для интерфейса `CsvWriter` разделитель может быть произвольной строкой. При использовании для интерфейса `CsvReader` должен быть однобайтным символом. Возвращает `this`.
