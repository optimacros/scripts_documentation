...
apiServiceRequestInfo(): ApiService.RequestInfo | null;
```
Возвращает ссылку на интерфейс [`ApiService.RequestInfo`](./apiService.md#RequestInfo), если скрипт вызван через API Service, иначе возвращает `null`.
...
log(message: string, print?: boolean): RequestManager
```
Выводит сообщение `message` в лог, доступ к которому можно получить в админпанеле. Если `print === true` (по умолчанию: `false`), дублирует сообщение `message` в консоль и дополнительно переносит курсор на новую строку. *Устаревшая функция.*

![Лог в админпанеле](./pic/requestInfo.png)
...
backup(path: string): EntityInfo|boolean
```
Сохраняет резервную копию в логах модели ->`Центр безопасности`->`Логи`->`Резервные копии`. Если прописать путь `path`, вызовется метод `export()`. Возвращает сущность резервной копии в виде [`EntityInfo`](./views.md#EntityInfo), или `true`, если был передан `path` .
...
actionsInfo(): ResultActionsInfo
```
Возвращает интерфейс [`ResultActionsInfo`](./scriptChains.md#ResultActionsInfo) создания действий, которые можно автоматически осуществить после исполнения текущего скрипта.
