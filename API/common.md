...
createKey(password: string): string
```
Создаёт ключ на основе пароля, которым можно в дальнейшем подписать лицензию. Повторный вызов приводит к генерации нового ключа.
...
validateKey(password: string, key: string): boolean
```
Проверяет ранее созданный ключ на соответствие паролю. Возвращает `true`, если проверка пройдена, иначе выбрасывает исключение.
...
createLicense(password: string, key: string, jsonStr: string): string
```
Создаёт лицензию — зашифрованные данные на основе пароля, ключа и JSON-данных лицензии. Возвращает строку с лицензией. В случае несоответствия ключа паролю или при передаче невалидной `JSON`-строки возникает ошибка интепретатора, которую нельзя отловить внутри скрипта.
...
validateLicense(password: string, key: string, licenseData: string, id?: string | null): object
```
Проверяет ранее созданную лицензию на соответствие паролю и ключу. Возвращает стандартный JS-объект с полями, указанными в исходной структуре лицензии, но у каждого названия свойства объекта появляется префикс `$`. В случае несоответствия пароля ключу возникает ошибка интепретатора, которую нельзя отловить внутри скрипта. В случае несоответствия лицензии ключу и паролю возвращается объект с текстом ошибки: `{"$errors":"Содержание лицензии не распознается"}`.
