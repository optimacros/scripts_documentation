...
numeric(): NumericElementsCreator
```
Возвращает ссылку на [`NumericElementsCreator`](#NumericElementsCreator).
...
named(): NamedElementsCreator
```
Возвращает ссылку на [`NamedElementsCreator`](#NamedElementsCreator).
...
appendIdentifier(identifier: number): ElementsDeleter
```
Добавляет в буфер элемент, чей [`longId`](./views.md#longId) равен `identifier`. Возвращает `this`.
