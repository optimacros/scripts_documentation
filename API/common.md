...
getWorkspaceContractInfo(): string;
```
Сравнивает параметры договора, указанные в настройках воркспейса (`Settings` -> `Editor` -> `Workspace` -> `Contract`), с текущими параметрами воркспейса. Возвращает строку — JSON-encoded объект, содержащий параметры договора и информацию об обнаруженных проблемах в дополнительном свойстве `errors`.
...
Если параметры договора не указаны для воркспейса в интерфейсе администратора, возвращаемый объект будет содержать только ошибку `Contract not found` в свойстве `erros`.
...
Если JSON-строка и значение хэш-функции не соответствуют "вшитому" в воркспейс значению `salt`, возвращаемый объект будет содержать только ошибку `Contract data does not match hash` в свойстве `erros`.

Если проверки, указанные выше, пройдены, но параметры, указанные в JSON-строке не соответствуют текущим параметрам воркспейса, возвращаемый JSON-encoded объект в дополнение к указанным в панели администратора свойствам будет содержать в дополнительном свойстве `errors` указание на эти несоответствия.

Если не обнаружено никаких проблем, то возвращаемый JSON-encoded объект будет идентичен тому, что указано в панели администратора воркспейса.
