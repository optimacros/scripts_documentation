...
Если JSON-строка и значение хэш-функции не соответствуют "вшитому" в воркспейс значению `salt`, возвращаемый объект будет содержать только ошибку `Contract data does not match hash`.

Если проверки, указанные выше, пройдены, но параметры, указанные в JSON-строке не соответствуют текущим параметрам воркспейса, возвращаемый объект `ContractInfo` будет содержать информацию о этих несоответствиях, которую моно получить, вызвав метод `errors()`.
