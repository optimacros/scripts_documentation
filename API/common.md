...
lastApplyErrors(): CellApplyError[];
```
Если после последнего выполнения функции [`apply()`](#cell-buffer.apply) в некоторые ячейки по каким-то причинам фактически не произошла запись, информация о каждой такой ячейке и причине неуспеха помещается в интерфейс [`CellApplyError`](#cell-apply-error). Функция возвращает массив таких интерфейсов.
...
Таким образом можно скрыть модальное окно после выполнения скрипта, установив свойство `lockModal` в значение `true`.
