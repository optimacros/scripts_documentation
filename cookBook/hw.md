...
Для начала нужно открыть список моделей: `Меню пользователя` > `Прочие возможности` > `Админка воркспейса` > `Models`. Затем найти нужную модель, нажать `Open` и в открывшейся модели перейти в раздел `JS Script`. Здесь настроена IDE для написания и запуска скриптов:
