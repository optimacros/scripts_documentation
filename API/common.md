...
getCurrentMacrosStorageReadMode(): string
```
Возвращает режим чтения данных в модели для текущего скрипта. Возвращаемое значение может быть одно из `'CONSISTENT_READ'`, `'FAST_READ'`, `'FAST_READ_METADATA'`.
...
setModelStorageReadMode(type: string): boolean;
```
Устанавливает режим чтения данных в модели. Аналог в интерфейсе `Optimacros`: `Меню пользователя` -> `Параметры` -> `Расширенные` -> `Режим чтения`. Параметр `type` принимает одно из значений `'CONSISTENT_READ'`, `'FAST_READ'`, `'FAST_READ_METADATA'`. Режим `'CONSISTENT_READ'` гарантирует, что при каждом запросе пользователю (или скрипту) будут возвращены данные со всеми модификациями, которые были сделаны до момента запроса, `'FAST_READ'` позволяет получить данные, не дожидаясь того, как порождённые предыдущими запросами пересчёты формул закончатся, `'FAST_READ_METADATA'` позволяет ещё и не ждать результатов изменений метаданных. Возвращает `true`.
...
getStorageReadMode(): string;
```
Возвращает установленный режим чтения данных в модели (одно из значений `'CONSISTENT_READ'`, `'FAST_READ'`, `'FAST_READ_METADATA'`).
...
getStorageWriteMode(): string;
```
Возвращает установленный режим записи данных в модели (одно из значений `'CONSISTENT_WRITE'`, `'FAST_WRITE'`).
...
setMacrosStorageReadMode(type: string): boolean
```
Устанавливает режим чтения данных в модели для скриптов. Аналог в интерфейсе `Optimacros`: `Меню пользователя` -> `Параметры` -> `Режимы чтения и записи` -> `Режим чтения для скриптов`. Параметр `type` принимает одно из значений `'CONSISTENT_READ'`, `'FAST_READ'`, `'FAST_READ_METADATA'`. Режим `'CONSISTENT_READ'` гарантирует, что при каждом запросе скрипту будут возвращены данные со всеми модификациями, которые были сделаны до момента запроса, `'FAST_READ'` позволяет получить данные, не дожидаясь того, как порождённые предыдущими запросами пересчёты формул закончатся, `'FAST_READ_METADATA'` позволяет ещё и не ждать результатов изменений метаданных. Метод ожидает завершения всех запросов в модели и только после переключает режим. Возвращает `true`.
...
getMacrosStorageReadMode(): string
```
Возвращает установленный режим чтения данных в модели для скриптов (одно из значений `'CONSISTENT_READ'`, `'FAST_READ'`, `'FAST_READ_METADATA'`).
