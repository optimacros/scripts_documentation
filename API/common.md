...
setCurrentMacrosStorageReadMode(type: string): boolean
```
Устанавливает режим чтения данных модели для текущего скрипта. Параметр `type` принимает одно из значений `'CONSISTENT_READ'`, `'FAST_READ'`, `'FAST_READ_METADATA'`. Режим `'CONSISTENT_READ'` гарантирует, что при каждом запросе текущему скрипту будут возвращены данные со всеми модификациями, которые были сделаны до момента запроса, `'FAST_READ'` позволяет получить данные, не дожидаясь того, как порождённые предыдущими запросами пересчёты формул закончатся, `'FAST_READ_METADATA'` позволяет ещё и не ждать результатов изменений метаданных. Возвращает `true`.
...
getCurrentMacrosStorageReadMode(): string
```
Возвращает режим чтения данных модели для текущего скрипта. Возвращаемое значение может быть одно из `'CONSISTENT_READ'`, `'FAST_READ'`, `'FAST_READ_METADATA'`.
...
setModelStorageReadMode(type: string): boolean;
```
Устанавливает режим чтения данных модели. Аналог в интерфейсе `Optimacros`: `Меню пользователя` -> `Параметры` -> `Расширенные` -> `Режим чтения`. Параметр `type` принимает одно из значений `'CONSISTENT_READ'`, `'FAST_READ'`, `'FAST_READ_METADATA'`. Режим `'CONSISTENT_READ'` гарантирует, что при каждом запросе пользователю (или скрипту) будут возвращены данные со всеми модификациями, которые были сделаны до момента запроса, `'FAST_READ'` позволяет получить данные, не дожидаясь того, как порождённые предыдущими запросами пересчёты формул закончатся, `'FAST_READ_METADATA'` позволяет ещё и не ждать результатов изменений метаданных. Возвращает `true`.
...
setModelStorageWriteMode(type: string): boolean;
```
Устанавливает режим записи данных в модель. Аналог в интерфейсе `Optimacros`: `Меню пользователя` -> `Параметры` -> `Расширенные` -> `Режим записи`. Параметр `type` принимает одно из значений `'CONSISTENT_WRITE'`, `'FAST_WRITE'`. Режим `'CONSISTENT_WRITE'` гарантирует, что при модификации данных ответ на запрос о модификации вернётся после того, как все ранее порождённые пересчёты формул будут завершены. `'FAST_WRITE'` позволяет не ждать завершения пересчётов при модификации данных, а сразу продолжать работу. Возвращает `true`.
...
getStorageReadMode(): string;
```
Возвращает установленный режим чтения данных модели (одно из значений `'CONSISTENT_READ'`, `'FAST_READ'`, `'FAST_READ_METADATA'`).
...
getStorageWriteMode(): string;
```
Возвращает установленный режим записи данных в модель (одно из значений `'CONSISTENT_WRITE'`, `'FAST_WRITE'`).
...
setMacrosStorageReadMode(type: string): boolean
```
Устанавливает режим чтения данных модели для скриптов. Аналог в интерфейсе `Optimacros`: `Меню пользователя` -> `Параметры` -> `Режимы чтения и записи` -> `Режим чтения для скриптов`. Параметр `type` принимает одно из значений `'CONSISTENT_READ'`, `'FAST_READ'`, `'FAST_READ_METADATA'`. Режим `'CONSISTENT_READ'` гарантирует, что при каждом запросе скрипту будут возвращены данные со всеми модификациями, которые были сделаны до момента запроса, `'FAST_READ'` позволяет получить данные, не дожидаясь того, как порождённые предыдущими запросами пересчёты формул закончатся, `'FAST_READ_METADATA'` позволяет ещё и не ждать результатов изменений метаданных. Метод ожидает завершения всех запросов в модели и только после переключает режим. Возвращает `true`.
...
getMacrosStorageReadMode(): string
```
Возвращает установленный режим чтения данных модели для скриптов (одно из значений `'CONSISTENT_READ'`, `'FAST_READ'`, `'FAST_READ_METADATA'`).
