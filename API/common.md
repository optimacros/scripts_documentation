...
Интерфейс предоставляет средства для генерации значения `salt`, предназначенное для вшивания в дистрибутив, который будет установлен на сервере клиента. Отдельно клиенту передается `JSON`-строка с параметрами договора и хэш, полученный от этой строки с использованием `salt`. Данные параметры клиент должен ввести в панели администратора воркспейса в специальное поле в настройках воркспейса.
