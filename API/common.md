...
recalculateIfManualCalculable(identifiers: number[]): boolean
```
Производит пересчёт сущностей, если на них не стоит флаг автокалькуляции. Автоматически пересчитываемые сущности пропускаются. При успешном пересчёте и при пустом списке сущностей возвращает `true`, и это **поведение отличается** от поведения группы функций [`recalculateCubes...()`](#model-info.recalculate-cubes).
