...
1. при сборке дистрибутива для клиента в него "зашивается" секретная строка (далее `salt`),
2. параметры договора записываются в виде JSON-строки фиксированного формата (который на данный момент не задокументирован),
3. данная строка и значение хэш-функции (тоже текст), которое было рассчитано, исходя из JSON-строки и `salt`, передаются клиенту,
4. клиент указывает оба этих значения в настройках воркспейса в панели администратора воркспейса,
5. воркспейс проверяет
...
6. в случае наличия ошибок или нарушения параметров договора всем пользователям выводится системное сообщение с информацией о проблеме.

Данный интерфейс предоставляет возможность работать с текущими параметрами договора воркспейса, а также генерировать новые пары `salt` — значение хэш-функции на основе произвольных параметров договора.
...
getWorkspaceContractInfo(): ContractInfo;
```
Сравнивает параметры договора, указанные в настройках воркспейса (`Settings` -> `Editor` -> `Workspace` -> `Contract`), с текущими параметрами воркспейса. Возвращает объект `ContractInfo`, который содержит информацию о параметрах договора (JSON-строка, возвращаемая методом `contractJson()`) и информацию об обнаруженных проблемах (список строк, возвращаемый методом `errors()`).

Метод `contractJson()` возвращаемого объекта `ContractInfo` вернет нормализованную JSON-строку, которая соответствует JSON-объекту, указанному в панели администратора воркспейса. Это нужно учитывать, т.к. если в панели администратора введена строка вида `{   "id"  :   "some_id"   }`, метод `contractJson()` вернет `{"id":"some_id"}`.
...
Если параметры договора не указаны для воркспейса в интерфейсе администратора, возвращаемый объект будет содержать только ошибку `Contract not found`.

Перед сверкой параметров, указанных в интерфейсе администратора, JSON-объект с параметрами договора проверяется аналогично `validateContractJson()`. Если указанный в интерфейсе администратора JSON-объект содержит синтаксическую ошибку или не соответствует схеме данных, будет выброшено соответствующее исключение.

Если JSON-строка и значение хэш-функции не соответствуют "вшитому" в воркспейс значению `salt`, возвращаемый объект будет содержать только ошибку `Contract data does not match hash`, а `contractJson()` вернёт пустой JSON-объект.

Если проверки, указанные выше, пройдены, но параметры, указанные в JSON-строке не соответствуют текущим параметрам воркспейса, возвращаемый объект `ContractInfo` будет содержать информацию об этих несоответствиях, которую можно получить, вызвав метод `errors()`.
...
calculateContractHash(contractData: string, salt: string): string;
```
Проверяет JSON-строку `contractData` с параметрами договора аналогично методу `validateContractJson()`, затем рассчитывает и возвращает значение хэш-функции, используя значение `salt`, которое должно быть произвольной непустой строкой.
...
`hash` и `salt` должны быть непустыми строками.
