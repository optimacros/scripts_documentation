...
wait(wait: number): ResultTaskAction|null
```
Устанавливает время ожидания разрешения промиса в секундах. Если установить время ожидпния `0`, то метод будет ждать завершения задачи. Возвращает [`ResultTaskAction`](#result-task-action), если ответ пришёл в рамках установленного времени, иначе `null`.
