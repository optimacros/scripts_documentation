...
```
Интерфейс для получения информации о загруженных клиентом файлах. Если требуется загрузить несколько файлов в одном ключе, в запросе для каждого файла нужно указывать одинаковый ключ-массив вида `key[]`. 
