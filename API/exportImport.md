...
Для форматов `'xls'` и `'xlsx'` нужно устанавливать разделитель `.`, иначе Excel трактует значение как текст. Отображаемый разделитель в Excel зависит от системных настроек компьютера и настроек самого Excel.
...
- Уровни иерархии справочников визуально выделяются "лесенкой" по аналогии с веб-интерфейсом. Импорт такого файла с итоговыми строками обратно в OM на данный момент недоступен.
...
Допустим лишь один однобайтовый символ. Значение по умолчанию: `пустая строка`. Возвращает `this`.
...
getFilePath(): string;
```
Возвращает имя импортируемого файла. Если имя ещё не задано, бросает исключение.
...
import(): this;
```
Производит импорт в [`Grid`](./views.md#grid) в соответствии с настройками. Возвращает `this`. Если имя файла ещё не задано, бросает исключение.
