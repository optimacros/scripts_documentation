...
createLicense(password: string, key: string, jsonStr: string): string
```
Создаёт лицензию — зашифрованные данные на основе пароля `password`, ключа `key` и JSON-данных лицензии `jsonStr`. Возвращает строку с лицензией. В случае несоответствия ключа паролю или при передаче невалидной `JSON`-строки возникает ошибка интепретатора, которую нельзя отловить внутри скрипта.
...
validateLicense(password: string, key: string, licenseData: string, id?: string | null): object
```
Проверяет ранее созданную лицензию `licenseData` на соответствие паролю `password` и ключу `key`. Возвращает объект с полями, указанными в исходной структуре лицензии, но у каждого названия свойства объекта появляется префикс `$`. В случае несоответствия пароля ключу возникает ошибка интепретатора, которую нельзя отловить внутри скрипта. В случае несоответствия лицензии ключу и паролю возвращается объект с текстом ошибки: `{"$errors": "Содержание лицензии не распознается"}`.
